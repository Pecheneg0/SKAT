#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Тестовый скрипт для проверки распознавания букв и расчета координат.
Работает с записанным видеофайлом вместо камеры.
Подключается к SITL для получения телеметрии.
"""

import cv2
import numpy as np
import time
import torch
import math
import logging
import sys
import os
from datetime import datetime
from torchvision import transforms
from PIL import Image
from pymavlink import mavutil

# === КОНФИГУРАЦИЯ ===
VIDEO_PATH = "flight_test.mp4"  # Путь к вашему видео
MAVLINK_PORT = "udpin:14551"  # Порт для подключения к SITL (или tcp:127.0.0.1:5762)
MAVLINK_BAUD = 57600

MODEL_PATH = "ALM_best (1).pth"
LABELS_PATH = "labels.txt"
CSV_OUTPUT_PATH = "/home/skatwsl/test/test_results.csv"

CONFIDENCE_THRESHOLD = 0.7
PIXELS_PER_METER = 44.482
ANOMALOUS_HEIGHT = 14.5  # Поправка на геоид (Москва)
MAX_ALTITUDE_M = 150.0

# Настройки логирования
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler("test_letters.log"),
        logging.StreamHandler(sys.stdout)
    ]
)

class LetterDetectorTest:
    def __init__(self):
        self.model = None
        self.labels = []
        self.master = None
        self.transform = None
        self.letter_stats = {}
        self.last_save_time = 0
    
    def bar_alt(self):
    
        msg = self.master.recv_match(type="GLOBAL_POSITION_INT", blocking=True)
        return msg.relative_alt / 1000


    def connect_to_sitl(self):
        """Подключение к симулятору SITL"""
        logging.info(f"Подключение к SITL: {MAVLINK_PORT}")
        try:
            self.master = mavutil.mavlink_connection("udpin:127.0.0.1:14551")
            self.master.wait_heartbeat()
            logging.info("Подключение к SITL успешно!")
            return True
        except Exception as e:
            logging.error(f"Ошибка подключения к SITL: {e}")
            logging.warning("Работа без телеметрии (координаты будут нулевыми)")
            return False
    
    def load_model(self):
        """Загрузка модели и весов"""
        try:
            # Импортируем архитектуру модели (должна быть в том же каталоге)
            from modelold import ArmenianLetterNet
            
            self.model = ArmenianLetterNet()
            self.model.load_state_dict(torch.load(MODEL_PATH, map_location=torch.device('cpu'), weights_only=False))
            self.model.eval()
            
            with open(LABELS_PATH, "r", encoding="utf-8") as f:
                self.labels = [line.strip() for line in f.readlines()]
            
            self.transform = transforms.Compose([
                transforms.Grayscale(num_output_channels=1),
                transforms.Resize((32, 32)),
                transforms.ToTensor(),
                transforms.Normalize((0.5,), (0.5,))
            ])
            logging.info("Модель загружена успешно")
            return True
        except Exception as e:
            logging.error(f"Ошибка загрузки модели: {e}")
            return False
    
    def get_telemetry(self):
        """Получение телеметрии от SITL"""
        if not self.master:
            return 55.754066, 37.617498, 30.0, 30.0, 0.0  # Дефолтные координаты (Москва)
        
        try:
            msg = self.master.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
            if msg:
                lat = msg.lat / 1e7
                lon = msg.lon / 1e7
                alt_agl = msg.relative_alt / 1e3
                alt_amsl = msg.alt / 1e3
                
                msg_att = self.master.recv_match(type='ATTITUDE', blocking=True)
                yaw = math.degrees(msg_att.yaw) if msg_att else 0.0
                
                return lat, lon, alt_agl, alt_amsl, yaw
            return 55.754066, 37.617498, 30.0, 30.0, 0.0
        except Exception as e:
            logging.error(f"Ошибка получения телеметрии: {e}")
            return 55.754066, 37.617498, 30.0, 30.0, 0.0
    
    def geo_to_3d_xyz(self, lat_deg, lon_deg, h_ellipsoid):
        """Прямое геодезическое преобразование (WGS-84)"""
        a = 6378137.0
        f = 1/298.257223563
        e2 = 2*f - f**2
        
        B = math.radians(lat_deg)
        L = math.radians(lon_deg)
        
        N = a / math.sqrt(1 - e2 * math.sin(B)**2)
        X = (N + h_ellipsoid) * math.cos(B) * math.cos(L)
        Y = (N + h_ellipsoid) * math.cos(B) * math.sin(L)
        Z = ((1 - e2) * N + h_ellipsoid) * math.sin(B)
        return X, Y, Z
    
    def xyz_to_geo_bowring(self, X, Y, Z):
        """Обратное геодезическое преобразование (Метод Боуринга)"""
        a = 6378137.0
        f = 1/298.257223563
        e2 = 2*f - f**2
        b = a * math.sqrt(1 - e2)
        ep2 = e2 / (1 - e2)
        
        Q = math.sqrt(X**2 + Y**2)
        if Q == 0:
            return (90.0 if Z > 0 else -90.0), 0.0, (abs(Z) - b)
        
        r = math.sqrt(Z**2 + Q**2 * (1 - e2))
        num = r**3 + b * ep2 * Z**2
        den = r**3 - b * e2 * (1 - e2) * Q**2
        
        B_rad = math.atan((Z / Q) * (num / den))
        L_rad = math.atan2(Y, X)
        
        N = a / math.sqrt(1 - e2 * math.sin(B_rad)**2)
        H = Q / math.cos(B_rad) - N
        
        return math.degrees(B_rad), math.degrees(L_rad), H
    
    def add_meters_to_coords(self, lat, lon, dx_m, dy_m, yaw_deg=0, alt_amsl=0):
        """Расчет координат объекта со смещением"""
        yaw_rad = math.radians(yaw_deg)
        
        # Поворот в систему Север-Восток
        north_m = dy_m * math.cos(yaw_rad) - dx_m * math.sin(yaw_rad)
        east_m = dy_m * math.sin(yaw_rad) + dx_m * math.cos(yaw_rad)
        
        # Высота эллипсоида
        h_ellipsoid = alt_amsl + ANOMALOUS_HEIGHT
        
        # XYZ дрона
        X, Y, Z = self.geo_to_3d_xyz(lat, lon, h_ellipsoid)
        
        # Смещение в XYZ
        lat_rad = math.radians(lat)
        lon_rad = math.radians(lon)
        
        dX = -math.sin(lat_rad)*math.cos(lon_rad)*north_m - math.sin(lon_rad)*east_m
        dY = -math.sin(lat_rad)*math.sin(lon_rad)*north_m + math.cos(lon_rad)*east_m
        dZ = math.cos(lat_rad)*north_m
        
        # Обратный перевод в геодезические
        new_lat, new_lon, _ = self.xyz_to_geo_bowring(X + dX, Y + dY, Z + dZ)
        
        return new_lat, new_lon
    
    def pixels_to_meters(self, pixel_x, pixel_y, ppm):
        """Перевод пикселей в метры"""
        return pixel_x / ppm, pixel_y / ppm
    
    def process_frame(self, frame, lat, lon, alt_agl, alt_amsl, yaw):
        """Обработка кадра: детекция + расчет координат"""
        if alt_agl > MAX_ALTITUDE_M:
            return []
        
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        _, thresh = cv2.threshold(gray, 180, 255, cv2.THRESH_BINARY)
        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        alt_safe = max(alt_agl, 1.0)
        _scale_sq = (30.0 / alt_safe) ** 2
        dynamic_min_area = max(11000 * _scale_sq, 50)
        dynamic_max_area = 400000 * _scale_sq
        
        results = []
        for cnt in contours:
            area = cv2.contourArea(cnt)
            if not (dynamic_min_area < area < dynamic_max_area):
                continue
            
            perimeter = cv2.arcLength(cnt, True)
            circularity = 4 * np.pi * area / (perimeter ** 2) if perimeter > 0 else 0
            if circularity < 0.4:
                continue
            
            rect = cv2.minAreaRect(cnt)
            box = cv2.boxPoints(rect)
            box = np.intp(box)
            center_x, center_y = int(rect[0][0]), int(rect[0][1])
            
            # Смещение от центра кадра
            frame_center_x, frame_center_y = 720/2, 720/2
            dx_px = center_x - frame_center_x
            dy_px = -center_y + frame_center_y
            
            # Масштаб
            dynamic_ppm = PIXELS_PER_METER * (30.0 / alt_safe)
            raw_dx_m, raw_dy_m = self.pixels_to_meters(dx_px, dy_px, dynamic_ppm)
            
            # Коррекция на наклон камеры (45 град)
            dx_m = raw_dx_m
            dy_m = raw_dy_m + alt_agl
            
            # Расчет геодезических координат
            new_lat, new_lon = self.add_meters_to_coords(lat, lon, dx_m, dy_m, yaw_deg=yaw, alt_amsl=alt_amsl)
            
            results.append({
                'center_px': (center_x, center_y),
                'box': box,
                'coords': (new_lat, new_lon),
                'alt': alt_agl,
                'dx_m': dx_m,
                'dy_m': dy_m
            })
        
        return results
    
    def recognize_letter(self, frame, box):
        """Распознавание буквы в контуре"""
        try:
            width, height = map(int, cv2.minAreaRect(box)[1])
            if width < 40 or height < 40:
                return None, 0.0
            
            # Перспективное выравнивание
            letter_crop = cv2.warpPerspective(
                frame,
                cv2.getPerspectiveTransform(
                    box.astype("float32"),
                    np.array([[0, height-1], [0, 0], [width-1, 0], [width-1, height-1]], dtype="float32")
                ),
                (width, height)
            )
            
            if np.mean(letter_crop) < 250:
                img_tensor = self.transform(Image.fromarray(letter_crop)).unsqueeze(0)
                
                with torch.no_grad():
                    output = self.model(img_tensor)
                    conf, pred = torch.max(torch.nn.functional.softmax(output, dim=1), 1)
                    
                    if conf.item() > CONFIDENCE_THRESHOLD:
                        label = self.labels[pred.item()]
                        parts = label.strip().split()
                        letter_char = parts[1] if len(parts) >= 2 else label
                        letter_idx = parts[0] if len(parts) >= 1 else "?"
                        return letter_char, conf.item()
        except Exception as e:
            logging.error(f"Ошибка распознавания: {e}")
        
        return None, 0.0
    
    def save_to_csv(self, letter_idx, lat, lon, conf):
        """Запись результата в CSV"""
        lat_e7 = int(round(lat * 1e7))
        lon_e7 = int(round(lon * 1e7))
        
        with open(CSV_OUTPUT_PATH, 'a', newline='', encoding='utf-8') as f:
            f.write(f"{letter_idx},{lat_e7},{lon_e7},{conf:.4f},{datetime.now().strftime('%H:%M:%S')}\n")
        
        logging.info(f"CSV: {letter_idx} | {lat:.7f}, {lon:.7f} | Conf: {conf:.2f}")
    
    def run(self):
        """Основной цикл обработки видео"""
        if not self.load_model():
            return
        
        self.connect_to_sitl()
        
        if not os.path.exists(VIDEO_PATH):
            logging.error(f"Видеофайл не найден: {VIDEO_PATH}")
            return
        
        cap = cv2.VideoCapture(VIDEO_PATH)
        if not cap.isOpened():
            logging.error("Не удалось открыть видеофайл")
            return
        
        fps = cap.get(cv2.CAP_PROP_FPS)
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        logging.info(f"Видео: {total_frames} кадров, {fps:.1f} FPS")
        
        frame_count = 0
        detections = {}
        
        try:
            while True:
                ret, frame = cap.read()
                if not ret:
                    break
                
                frame_count += 1
               # if frame_count % 10 != 0:  # Пропускаем кадры для скорости
                #    continue
                
                # Телеметрия
                lat, lon, alt_agl, alt_amsl, yaw = self.get_telemetry()
                
                # Детекция
                results = self.process_frame(frame, lat, lon, alt_agl, alt_amsl, yaw)
                
                for result in results:
                    letter, conf = self.recognize_letter(frame, result['box'])
                    if letter:
                        key = f"{letter}_{result['coords']}"
                        if key not in detections:
                            detections[key] = {'count': 0, 'coords': result['coords'], 'conf': conf}
                            # Извлекаем индекс буквы из labels
                            label_full = [l for l in self.labels if letter in l]
                            idx = label_full[0].split()[0] if label_full else "0"
                            self.save_to_csv(idx, result['coords'][0], result['coords'][1], conf)
                            logging.info(f"ОБНАРУЖЕНО: {letter} @ {result['coords']}")
                        detections[key]['count'] += 1
                
                # Визуализация (опционально)
                cv2.putText(frame, f"Frame: {frame_count}/{total_frames}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(frame, f"Alt: {alt_agl:.1f}m", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                cv2.imshow("Test Detection", frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
                    
        finally:
            cap.release()
            cv2.destroyAllWindows()
            logging.info(f"Обработка завершена. Всего обнаружений: {len(detections)}")

if __name__ == "__main__":
    detector = LetterDetectorTest()
    detector.run()
